...
const Header = () => {
  ...
  useEffect(() => {
    localStorage.setItem("MenuOpened", JSON.stringify(open));
  }, [open]);

  return (
    <div id="navbar" className="navbar-box sticky top-0 z-50">
      <div
        className={`
        ${open ? "navbar active" : "navbar"} 
        ${bg || open ? "backdrop-filter backdrop-blur-lg bg-opacity-30 h-16 shadow-md shadow-indigo-200/10" : "h-24"} 
         w-full h-full lg:flex lg:items-center lg:justify-between items-center`}
      >
        <div className="list-container container mx-auto flex h-full items-center justify-between px-20">
          <Fade direction="down" cascade duration={150} triggerOnce="true">
            <div className="logo flex items-center h-[64px]">
              <Link to="about" className="cursor-pointer" activeClass="active" spy={true} smooth={true} duration={700} offset={-150}>
            ...
            <Fade direction="down" delay={100} cascade duration={150} triggerOnce="true">
              <ul className="navlink text-white flex justify-center items-center lg:space-x-12 capitalize text-[15px]">
                {navigation.map((item, index) => {
                  return (
                    <li
                      ...
                      key={index}
                    >
                      <Link onClick={closeNavbar} to={item.href} activeClass="active" spy={true} smooth={true} duration={700} offset={-150} className="transition-all duration-300">
                        {item.name}
                      </Link>
                    </li>
                  );
                ...
                <li className="text-white text-base font-normal cursor-pointer">
                  <a href={Resume} className="transition-all duration-300 border-2 px-4 py-2 hover:bg-indigo-600 text-indigo-300 hover:text-white border-indigo-600 rounded" target="_blank" rel="noreferrer">
                    Resume
                  </a>
                </li>
              </ul>
